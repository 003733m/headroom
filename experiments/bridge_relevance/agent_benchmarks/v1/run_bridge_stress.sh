...
ROOT="/tmp/headroom-bridge-stress-final"
...
BRIDGE-ELIGIBLE STRESS PROTOCOL:

Before making any code change:

1. Run the relevant failing regression test for this task.

2. Then run this exact shell command once:

rg -nH -e 'def |class |return |raise |assert |if ' headroom tests

Do not pipe, truncate, redirect, filter, or otherwise post-process that command's output.

3. Do not use the dedicated Grep tool anywhere in this task.
For any later repository-wide text search, use shell rg -nH instead.

4. After the required diagnostic search, continue debugging normally.
Use the evidence you observed, implement a minimal robust fix, and run the relevant tests.

The search-interface rule is part of the evaluation protocol; it is not a hint about the location or implementation of the fix.
"
...
echo "=================================================="
echo "PHASE 2 — BRIDGE-ELIGIBLE REAL-BUG AGENT A/B"
echo "=================================================="
...
md = []
md.append("# Bridge-eligible real-bug stress benchmark")
md.append("")
