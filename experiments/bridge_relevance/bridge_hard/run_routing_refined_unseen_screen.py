...
EXEC_REL = (
    ...
    "routing_refined_unseen_execution.json"
)
PROTOCOL_REL = (
    "experiments/bridge_relevance/bridge_hard/"
    "routing_refined_unseen_validation_protocol.json"
)
SELECTION_REL = (
    "experiments/bridge_relevance/bridge_hard/"
...
SNAP_LOCK_REL = (
    "experiments/bridge_relevance/bridge_hard/"
    "routing_refined_unseen_snapshot_lock.json"
)

FREEZE = "8fa4d92529f47174101e4d6fb288c0fde32e8f7c"

ROOT = Path.home() / "headroom-routing-refined-unseen-20260906"
RUNTIME = Path("/tmp/headroom-routing-refined-runtime-8fa4d925")
SNAPSHOTS = Path("/tmp/headroom-routing-refined-snapshots-20260906")
...
def locked_manifest():
    ...
    if d.get("task_count") != 11:
        die("expected exactly 11 tasks")

    expected = [f"U{i:02d}-OFF" for i in range(2, 13)]
...
def prepare_snapshots():
    ...
    lock = {
        "status": "prepared_before_any_routing_refined_unseen_agent_run",
        "experiment": "routing_refined_adopted_bridge_unseen_validation",
...
def snapshot_lock():
    ...
    if d.get("status") != "prepared_before_any_routing_refined_unseen_agent_run":
...
def preflight():
    ...
    print()
    print("=" * 72)
    print("ROUTING-REFINED UNSEEN PREFLIGHT PASSED")
    print("=" * 72)
    print("freeze :", FREEZE)
    print("tasks  : 11")
    print("next   : U02-OFF")
...
def run_next():
    ...
    for item in manifest["run_order"]:
        ...
        print()
        print("=" * 72)
        print("RUNNING ROUTING-REFINED UNSEEN SCREEN:", item)
        print("=" * 72)
...
def status():
    ...
    print("===== ROUTING-REFINED UNSEEN STATUS =====")
